...
Queue::~Queue()
{
	...
	else
	{
		...
		// Since one of the restriction is to not delete any of the node our starting location in the circle can be anywhere
		// We store the previous node the head points to, this lets us know when we fully traversed the circle
		while (head)
		{
			node* temp = head;
			head = head->next;
